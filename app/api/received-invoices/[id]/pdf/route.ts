...
export async function GET(req: NextRequest, { params }: { params: { id: string } }) {
  ...
  // 4. Plantilla HTML con estilos básicos (proveedor como emisor)
  ...
  <html>
    <head>
      <meta charset="utf-8" />
      <title>Factura Recibida #${invoice.number}</title>
      <style>
    ...
    <body>
      <div class="header">
        ...
        <div>
          <h2>Factura Recibida #${invoice.number}</h2>
          <div>Fecha: ${invoice.date ? new Date(invoice.date).toLocaleDateString("es-ES") : "-"}</div>
        </div>
      </div>
      <div class="section">
        <div class="section-title">Proveedor (Emisor)</div>
        <div>${invoice.provider?.name || "-"}</div>
        <div>${invoice.provider?.address || ""}</div>
        <div>NIF: ${invoice.provider?.nif || ""}</div>
      </div>
      <div class="section">
        <div class="section-title">Negocio (Receptor)</div>
  ...
  try {
    ...
    return new Response(pdfBuffer, {
      headers: {
        ...
        "Content-Disposition": `attachment; filename=FacturaRecibida-${invoice.number}.pdf`,
      },
    })
  } catch (err) {
